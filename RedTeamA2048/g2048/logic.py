...
class GameBoard:
    ...
    def shift_horiz(cls, board, right):
        for y, row in enumerate(board):
            ...
            if right:
                for y in cls.translate_horiz(y, cls.translate_reversed(iterator)):
                    yield y
            else:
                for y in cls.translate_horiz(y, iterator):
                    yield y
    ...
    def shift_vert(cls, board, down):
        for x, col in enumerate(board.transpose()):
            ...
            if down:
                for y in cls.translate_vert(x, cls.translate_reversed(iterator)):
                    yield y
            else:
                for y in cls.translate_vert(x, iterator):
                    yield y
    ...
    def shift(self, direction):
        ...
        if actions:
            with open("shift", "w") as f:
                f.write(str(self.board.board))
                for action, p0, p1 in actions:
                    f.write("{} {} {}".format(action, p0, p1))
                    if action == "merge":
